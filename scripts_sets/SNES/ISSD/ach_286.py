...
# 2. CENÁRIOS (Normal & Hardcore - Bug do value(0) resolvido)
# (Idx, bit_func, mem_addr, P2_max_goals)
scenario_flags = [
    (0,  bit7, 0x00d88d, 2), (1,  bit6, 0x00d88d, 3), # bit6 resolve o bug do 0xS!
    (2,  bit5, 0x00d88d, 3), (3,  bit4, 0x00d88d, 0),
    (4,  bit3, 0x00d88d, 1), (5,  bit2, 0x00d88d, 1),
    (6,  bit1, 0x00d88d, 1), (7,  bit0, 0x00d88d, 0),
    (8,  bit7, 0x00d706, 1), (9,  bit6, 0x00d706, 2), # bit6 aqui também
...
for i in range(12):
    ...
    ach_norm.add_core([
        (mem_difficulty >= 0x02), (mem_p1_score > mem_p2_score),
        (mem_scenario_idx == idx), (mem_game_mode == 0x04),
        (bit_fn(addr) == 0x01), (bit_fn(addr).delta() == 0x00),
        cond_no_99_goals
    ])
    ...
    ach_hard.add_core([
        (mem_difficulty == 0x04), trigger(mem_p1_score > mem_p2_score),
        (mem_game_mode == 0x04), (mem_scenario_idx == idx),
        trigger(bit_fn(addr) == 0x01), (bit_fn(addr).delta() == 0x00),
        cond_no_99_goals,
        and_next(mem_scenario_idx == idx), pause_if((mem_p2_score > max_goals).with_hits(1)),
    ])
...
ach.add_core([
    (byte(0x00d88d) == 0xff).with_hits(1), (byte(0x00d706) == 0xf1).with_hits(1),
    (mem_game_mode == 0x04), (mem_difficulty >= 0x03),
    trigger(byte(0x0013e1) == 0x01), (byte(0x0013e1).delta() == 0x00),
    *(reset_if(byte(0x00d88e + j) > 1) for j in range(12))
])
my_set.add_achievement(ach)

# 3. TREINAMENTOS (Records)
training_data = [
...
for t_id, title, type_val, diff_val, target, addr, pts, badge in training_data:
    ach = Achievement(id=t_id, title=title, description="Break the training record", points=pts, badge=badge)
    ach.add_core([
        (mem_train_type == type_val), (mem_train_diff == diff_val), (mem_game_mode == 0x06),
        trigger(word(addr) > target), (word(addr).delta() <= target),
    ])
    my_set.add_achievement(ach)

# 4. TORNEIOS E WORLD SERIES
ws_data = [
    (623460, "International Superstar", "Win the International on Game Level 4 or above", 10, "708386", [or_next(mem_game_mode == 0x01), (mem_world_mode == 0x02), (mem_difficulty >= 0x03)]),
    (623461, "The World Is Yours", "Win the World Series during the first season with any team on Difficulty 4 or above", 10, "708387", [(mem_ws_wins == 0x23).with_hits(1), or_next(mem_game_mode == 0x02), reset_if(mem_world_mode != 0x03), (mem_difficulty >= 0x03)]),
    (10280, "World Champion", "Win the World Series without a single loss in the season", 25, "708455", [trigger((mem_ws_wins == 0x23).with_hits(1)), reset_if(byte(0x00dc60) != 0x00), or_next(mem_game_mode == 0x02), (mem_world_mode == 0x03)]),
    (10281, "Defying the Impossible", "Win every match in the World Series as Morocco", 25, "708456", [trigger((mem_ws_wins == 0x23).with_hits(1)), reset_if(byte(0x00dc60) != 0x00), (mem_p1_team == 0x3a), or_next(mem_game_mode == 0x02), (mem_world_mode == 0x03)]),
    (10282, "It Will Take Something Special", "Win every match in the World Series on Game Level 5", 50, "708457", [trigger((mem_ws_wins == 0x23).with_hits(1)), (mem_difficulty == 0x04), reset_if(byte(0x00dc60) != 0x00), or_next(mem_game_mode == 0x02), (mem_world_mode == 0x03)]),
]

for w_id, title, desc, pts, badge, conds in ws_data:
    ach = Achievement(id=w_id, title=title, description=desc, points=pts, badge=badge, type=AchievementType.WIN_CONDITION if pts==10 else AchievementType.STANDARD)
    ach.add_core([*conds, trigger(mem_ws_match_st == 0x01), (mem_ws_match_st.delta() != 0x01)])
    my_set.add_achievement(ach)
...
# 5. GOLS E DESAFIOS DE PARTIDA
# (ID, Title, Desc, Pts, Badge, Hits)
goals_data = [
    (534, "Yes! GOALLLL!!!", "Score your first goal in any game mode", 1, "708418", 1),
    (9370, "Hat Trick!", "Score 3 goals in a match", 3, "708419", 3),
    (535, "GOALLLL!!!! GOALLLLLL!!!!", "Score 10 goals in a match", 5, "708420", 10),
    (9371, "I Don't Believe It", "Score 20 goals in a match", 10, "708421", 20),
]

for m_id, title, desc, pts, badge, hits in goals_data:
    ach = Achievement(id=m_id, title=title, description=desc, points=pts, badge=badge)
    ach.add_core([
        goal_trigger(hits), (mem_screen != 0x03), (mem_match_active == 0x01),
        (mem_cpu_active == 0x00), (mem_p1_ctrl == 0x00), cond_no_99_goals
    ])
    if hits == 1:
        ach.add_core([reset_next_if(mem_match_active == 0x00)])
    else:
        ach.add_alt([reset_if(mem_match_active == 0x00)])
    my_set.add_achievement(ach)

# Condições Gerais e Faltas
...
ach.add_core([(mem_p1_score > mem_p2_score), (mem_match_active == 0x01), (mem_match_state == 0x14), (mem_cpu_active == 0x00), (mem_p1_ctrl == 0x00), (mem_match_state.delta() != 0x14), cond_no_99_goals])
ach.add_alt([reset_if(mem_match_active == 0x00)])
my_set.add_achievement(ach)

ach = Achievement(id=10276, title="Word of Warning", description="Receive a Yellow Card", points=1, badge="708423")
ach.add_core([(byte(0x000dae) > value(0)).with_hits(1), (mem_match_active == 0x01), (mem_cpu_active == 0x00), (mem_p1_ctrl == 0x00), cond_no_99_goals])
ach.add_alt([reset_if(mem_match_active == 0x00)])
my_set.add_achievement(ach)

ach = Achievement(id=10277, title="You're Off!", description="Receive a Red Card", points=1, badge="708424")
ach.add_core([(byte(0x000daf) > value(0)).with_hits(1), (mem_match_active == 0x01), (mem_cpu_active == 0x00), (mem_p1_ctrl == 0x00), cond_no_99_goals])
ach.add_alt([reset_if(mem_match_active == 0x00)])
my_set.add_achievement(ach)
...
ach.add_core([(byte(0x000daa) == 0x00), (mem_p1_score > mem_p2_score), (mem_match_active == 0x01), (mem_cpu_active == 0x00), (mem_p1_ctrl == 0x00), (mem_match_state == 0x14), (mem_match_state.delta() != 0x14), cond_no_99_goals])
ach.add_alt([reset_if(mem_match_active == 0x00)])
my_set.add_achievement(ach)
...
ach.add_core([measured((byte(0x000dac) > value(0)).with_hits(10)), (mem_match_active == 0x01), (mem_cpu_active == 0x00), (mem_p1_ctrl == 0x00), cond_no_99_goals])
ach.add_alt([reset_if(mem_match_active == 0x00)])
my_set.add_achievement(ach)
...
ach.add_core([measured((byte(0x000dab) > value(0)).with_hits(10)), (mem_match_active == 0x01), (mem_cpu_active == 0x00), (mem_p1_ctrl == 0x00), cond_no_99_goals, pause_if((byte(0x000dae) > 0x00).with_hits(1))])
ach.add_alt([reset_if(mem_match_active == 0x00)])
my_set.add_achievement(ach)
...
ach.add_core([(mem_p2_team == 0x48), (mem_match_active == 0x01), (mem_p1_ctrl == 0x00), (mem_cpu_active == 0x00), trigger(mem_p1_score > mem_p2_score), trigger(mem_match_state == 0x14), (mem_match_state.delta() != 0x14), cond_no_99_goals])
ach.add_alt([reset_if(mem_match_active == 0x00)])
my_set.add_achievement(ach)
...
ach.add_core([(mem_p1_team == 0x3a), (mem_p2_team == 0x48), (mem_match_active == 0x01), (mem_p1_ctrl == 0x00), (mem_cpu_active == 0x00), (byte(0x001f88) == 0x02), (mem_difficulty == 0x04), trigger(mem_p1_score > mem_p2_score), trigger(mem_match_state == 0x14), (mem_match_state.delta() != 0x14), cond_no_99_goals])
ach.add_alt([reset_if(mem_match_active == 0x00)])
my_set.add_achievement(ach)
...
ach.add_core([trigger((mem_p1_score > value(0)).with_hits(1)), (mem_difficulty >= 0x03), (mem_cpu_active == 0x00), (mem_p1_ctrl == 0x00), (mem_match_active == 0x01), (mem_match_state == 0x11).with_hits(1)])
ach.add_alt([reset_if(mem_match_state == 0x02), reset_if(mem_match_state == 0x2c), reset_if(mem_match_state == 0x03), reset_if(mem_match_state == 0x01), reset_if(byte(0x00104c) == 0x01)])
my_set.add_achievement(ach)
...
ach.add_core([trigger((mem_p1_score > value(0)).with_hits(1)), (mem_difficulty >= 0x03), (mem_cpu_active == 0x00), (mem_p1_ctrl == 0x00), (mem_match_active == 0x01), (mem_match_state == 0x3d).with_hits(1)])
ach.add_alt([reset_if(mem_match_state == 0x02), reset_if(mem_match_state == 0x03), reset_if(mem_match_state == 0x01), reset_if(mem_match_state == 0x1f), reset_if(mem_match_state == 0x18), reset_if(byte(0x00104c) == 0x01), reset_if(bit7(0x000021) == 0x01), reset_if(mem_match_active == 0x00)])
my_set.add_achievement(ach)

ach = Achievement(id=623465, title="Dead Ball Specialist", description="Score a goal from a free kick during a full match on Game Level 4 or above", points=3, badge="708391")
ach.add_core([trigger((mem_p1_score > value(0)).with_hits(1)), (mem_match_state == 0x04).with_hits(1), (mem_difficulty >= 0x03), (mem_cpu_active == 0x00), (mem_p1_ctrl == 0x00)])
ach.add_alt([reset_if(mem_match_state == 0x02), reset_if(mem_match_state == 0x2c), reset_if(mem_match_state == 0x03), reset_if(mem_match_state == 0x11), reset_if(byte(0x00104c) == 0x01), reset_if(mem_match_active == 0x00)])
my_set.add_achievement(ach)
